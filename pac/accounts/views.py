...
class InitRegisterView(APIView):
    ...
    def post(self, request):
        ...
        serializer = InitialRegisterSerializer(curUser, data=request.data, partial = True)
        if serializer.is_valid():
            serializer.save()
            curUser.is_registered = True
            curUser.save()
            return Response({ "success": True, "msg": "ok" }, status=status.HTTP_200_OK)
        else:
            if "user_id" in serializer.errors.keys():
                return Response({ "success": False, "msg": serializer.errors["user_id"][0] }, status=status.HTTP_200_OK)
            elif "username" in serializer.errors.keys():
                return Response({ "success": False, "msg": serializer.errors["username"][0] }, status=status.HTTP_200_OK)
...
class UserView(generics.ListAPIView):
    ...
    def get(self, request):
        ...
        if keyword:
            member_objects = memberObj.filter(is_active = True, is_registered = True).filter(Q(user_id__icontains = keyword)|Q(username__icontains = keyword))
        else:
            member_objects = memberObj.filter(is_active = True, is_registered = True)

        memberQuery = member_objects.all()[start_index:start_index + size]
        return Response(MainInfoSerializer(memberQuery, many = True).data, status=status.HTTP_200_OK)
...
def get_tree_list(cur_genre):
    ...
    if cur_genre.is_terminal():
        ...
        for child_user in Genre.objects.filter(parent = cur_genre).order_by('order'):
...
def get_friendship_user(request):
    ...
    if serializer.is_valid():
        ...
        if is_rank:
            ...
            if is_active:
                friendships = cur_user.following.filter(followee__is_active = True).order_by('-created_at').all()[start_index:start_index + size]
                returned_user = [ item.followee.id for item in friendships ]
            else:
                if is_new:
                    from datetime import datetime, timedelta
                    threshold_time = datetime.now() - timedelta(days = 3)
                    friendships = cur_user.followers.filter(created_at__gt = threshold_time, follower__is_active = True).order_by('-created_at').all()[start_index:start_index + size]
                else:
                    friendships = cur_user.followers.filter(follower__is_active = True).all()[start_index:start_index + size]

                returned_user = [ item.follower.id for item in friendships ]

        if is_rank:
            maininfo_serializer = MainInfoSerializer(Member.objects.filter(id__in = returned_user), many = True)
        else:
            maininfo_serializer = MainInfoSerializer(Member.objects.filter(id__in = returned_user).order_by('-created_at'), many = True)
...
def edit_profile(request):    
    ...
    if "user" in requestData.keys():
        ...
        # if email changed
        emailChanged = False
        if dataUser["email"] != None and dataUser["email"].strip() != "":
            newEmail = dataUser["email"].strip()
            if newEmail != curUser.email:
                try:
                    # temperary
                    user_result = fire_auth.update_user(curUser.firebase_id, email=newEmail, email_verified=False)
                    emailChanged = True
                except:
                    return Response({
        ...
        newUserSerialzier = MemberSerializer(curUser, data = requestData["user"], partial = True)
        if newUserSerialzier.is_valid():            
            newUserSerialzier.save()
            return Response({ "data": newUserSerialzier.data, "email_changed": emailChanged }, status = status.HTTP_200_OK)
        else:
            return Response({
...
def get_followers(request):
    ...
    maininfo_serializer = MainInfoSerializer(Member.objects.filter(id__in = followees), many = True)
    return Response(maininfo_serializer.data, status=status.HTTP_200_OK)
...
def follow_user(request, id = None):
    ...
    if user.id == follower_id:
        return Response("You can not follow yourself", status.HTTP_400_BAD_REQUEST)
    else:
        cur_count = Friendship.objects.filter(follower = user, followee = target_user).count()
        if cur_count > 0:
            return Response(status = status.HTTP_400_BAD_REQUEST)
        else:
...
def unfollow_user(request, id = None):
    ...
    if user.id == unfollower_id:
        return Response("You can not unfollow yourself", status.HTTP_400_BAD_REQUEST)
    else:
        cur_count = Friendship.objects.filter(follower = user, followee = target_user).count()
        if cur_count > 0:
            Friendship.objects.filter(follower = user, followee = target_user).delete()
            return Response({"success": True}, status.HTTP_200_OK)
        else:
            return Response(status = status.HTTP_400_BAD_REQUEST)
