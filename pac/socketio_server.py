...
def init(sid, data):
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM chats_message WHERE receiver_id = %s and is_read = 0", data['id'])
        result = cur.fetchone()
        sio.emit('message_count', { 'count': result[0] }, room = "firebase-{}".format(data['id']))
