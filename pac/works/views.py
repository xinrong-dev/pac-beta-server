...
def like_work(request, id = None):
    ...
    if user.id == cur_work.creator.id:
        return Response("You can not like your work", status.HTTP_400_BAD_REQUEST)
    else:
        cur_count = Favorite.objects.filter(liker = user, favorite = cur_work).count()
        if cur_count > 0:
            Favorite.objects.filter(liker = user, favorite = cur_work).delete()
            return Response({"success": True, "result": "minus"}, status.HTTP_200_OK)
        else:
            Favorite.objects.create(liker = user, favorite = cur_work)
            return Response({"success": True, "result": "plus"}, status.HTTP_200_OK)

class WorksDetail(APIView):
    def get_permissions(self):
        if self.request.method == "PUT" or self.request.method == "DELETE":
            self.permission_classes = [SuperUserOnlyPermission | IsOwnerPermission]
        else:
    ...
    def put(self, request, pk, format = None):
        ...
        if serializer.is_valid():
            ...
            return Response(WorkSerializer(cur_work).data, status=status.HTTP_200_OK)
        else:
        ...
        if serializer.is_valid():
            ...
            if creator > 0:
                if creator == user.id:
                    ...
                    work_queryset = Work.objects.filter(status = "public", creator__id = creator)
            else:
                followers_id = list(user.following.filter(followee__is_active = True).values_list('followee_id', flat = True))
                followers_id.append(user.id)
                work_queryset = Work.objects.filter(status = "public", creator__id__in = followers_id)

            if keyword:
                work_queryset = work_queryset.filter(title__icontains = "keyword").order_by("-created_at").all()[start_index:start_index + size]
            else:
                work_queryset = work_queryset.order_by('-created_at').all()[start_index:start_index + size]
...
def get_work_comment(request):
    ...
    if serializer.is_valid():
        ...
        comment_queryset = work_obj.comments.filter(commentor__is_active = True).order_by('-created_at').all()[start_index:start_index + size]

        return Response(CommentSerializer(comment_queryset, many = True).data, status=status.HTTP_200_OK)
    else:
        return Response(status=status.HTTP_400_BAD_REQUEST)
...
def get_related_tags(request):
    ...
    if query != "":
        tags = Tag.objects.filter(name__contains = query).values_list('name', flat = True)
...
class CommentView(APIView):
    ...
    def post(self, request):
        ...
        Comment.objects.create(work = cur_work, commentor = user, content = work_comment)
