...
class UserView(APIView):
    ...
    def get(self, request):
        ...
        input_serializer = ChatUserPaginationSerializer(
        ...
        if input_serializer.is_valid():
            ...
            if keyword == "":
                message_query = Message.objects.filter(Q(Q(receiver__is_active=True) & Q(sender__id=user.id)) | Q(
                    Q(receiver__id=user.id) & Q(sender__is_active=True))).order_by('-created_at')
            else:
                message_query = Message.objects.filter(
                    Q(
                        Q(
                            receiver__user_id__icontains=keyword) & Q(
                            receiver__is_active=True) & Q(
                            sender__id=user.id)) | Q(
                        Q(
                            sender__user_id__icontains=keyword) & Q(
                                receiver__id=user.id) & Q(
                                    sender__is_active=True))).order_by('-created_at')

            result = message_query.values('room_str').order_by(
                'room_str', '-created_at').annotate(
                unread=Count(
                    'room_str', filter=Q(
                        receiver_id=user.id) & Q(
                        is_read=0))).order_by()[
            ...
            for item in result:
                ...
                last_message = Message.objects.filter(
                    room_str=item['room_str']).order_by('-created_at').first()
...
class MessageView(APIView):
    ...
    def get(self, request):
        ...
        input_serializer = MessagePaginationSerializer(
            data=request.query_params)
        if input_serializer.is_valid():
            input_data = input_serializer.data
            keyword = ""
    ...
    def post(self, request):
        ...
        if int(target_id) > 0 and content.strip() != "":
            ...
            new_message = Message.objects.create(
                content=content, sender=request.user, receiver=target_user)
            return Response(
